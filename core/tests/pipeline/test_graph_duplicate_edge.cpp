...
int main()
{
    ...
    if (in_degrees.at("B") != 2)
    {
        std::cerr << "Expected duplicate edges to increment in-degree to 2." << std::endl;
        return 1;
    }
    ...
    if (res.is_err())
    {
        std::cerr << "Graph with duplicate edges should validate." << std::endl;
        return 1;
    }

    std::cout << "Graph duplicate edge test passed." << std::endl;
    return 0;
}
