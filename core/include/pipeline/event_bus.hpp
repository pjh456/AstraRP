...
namespace astra_rp
{
    namespace pipeline
    {
        ...
        template <>
        struct EventBus::EventTraits<TokenEvent>
        {
        ...
        template <>
        struct EventBus::EventTraits<StateEvent>
        {
        ...
        template <>
        struct EventBus::EventTraits<ErrorEvent>
        {
        ...
        inline auto EventBus::subscribers() -> SubscriberVecFor<Event> &
        {
            ...
            if constexpr (
                std::is_same_v<EventType, TokenEvent>)
            {
                return m_token_subs;
            }
            else if constexpr (
                std::is_same_v<EventType, StateEvent>)
            {
                return m_state_subs;
            }
            else
            {
                static_assert(
                    std::is_same_v<EventType, ErrorEvent>,
                    "Unsupported EventBus event type");
                return m_error_subs;
            }
        }

        template <typename Event>
        inline auto EventBus::subscribers() const
            -> const SubscriberVecFor<Event> &
        {
            using EventType = std::decay_t<Event>;
        ...
        template <typename Event>
        inline void EventBus::invoke(
            const CallbackFor<Event> &cb,
            const std::decay_t<Event> &event)
        {
            using EventType = std::decay_t<Event>;
            if constexpr (std::is_same_v<EventType, TokenEvent>)
            {
